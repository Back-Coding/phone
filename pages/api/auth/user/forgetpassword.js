...
const handler = async (req, res) => {
  if (req.method === 'POST') {
    ...
    try {
      ...
      const resetUrl = `${process.env.DOMAIN}/user-reset-password/${resetToken}`;
...
function sendEmail(email, resetUrl){
  try {
    ...
    const transporter = nodemailer.createTransport({
      ...
      auth: {
        type: "OAuth2",
        user: "backcoding.rupesh@gmail.com",
        clientId: CLIENT_ID,
    ...
    const mailOptions = {
      from: "BackCoding  <backcoding.rupesh@gmail.com>",
      to: email,
